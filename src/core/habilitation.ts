import { children_flattener } from "./childrenFlattener"
...
export class Habilitation {
    ...
    public static parseHabilitationResponse(response:string): HabilitationResponse {
		...
		try {
      if(tags.includes("CustomerPortal.Access") && document.querySelector("li:has(#nav-dashboard)")) {
      ...
      console.warn("[Habilitations] Too early to access the DOM");
    }
