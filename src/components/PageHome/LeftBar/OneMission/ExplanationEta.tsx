...
export const ExplanationEta = (props: {
    ...
    if (props.geolocation?.mission?.datetime && fns.isAfter(new Date(props.geolocation.mission.datetime), new Date()) && (isNaN(eta.getTime()) || eta.getTime() == 0)) {
        return <p style={{ fontSize:'smaller', color: 'red', marginRight: 30 }}>
            {t("noPositionForThisMission")}
        </p>
    }
    ...
    if (fns.isBefore(new Date(props.mission.date.substring(0,10) + "T" + props.mission.startTime + ':00'), new Date()) && fns.isBefore(eta, new Date())) {
        return <p style={{ fontSize:'smaller', color: 'green', marginRight: 30  }}>
